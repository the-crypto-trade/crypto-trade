...
class Bybit(Exchange):
    ...
    def __init__(self, **kwargs) -> None:
        ...
        #     self.websocket_market_data_base_url = "wss://wspap.bybit.com:8443"
        # self.websocket_account_base_url = self.websocket_market_data_base_url
        self.websocket_market_data_path = f"/v5/public/{self.instrument_type.value}"
    ...
    #     elif self.instrument_type == BybitInstrumentType.SWAP:
    ...
    #         query_params={"instType": f"{BybitInstrumentType.SPOT if self.instrument_type == BybitInstrumentType.MARGIN else self.instrument_type}"},
    #     )
    ...
    def is_websocket_response_success(self, *, websocket_message):
        payload_summary = websocket_message.payload_summary
        return payload_summary["success"]
    ...
    def convert_websocket_push_data_for_trade(self, *, json_deserialized_payload):
        ...
        return [self.convert_dict_to_trade(input=x, api_method=ApiMethod.WEBSOCKET, symbol=symbol) for x in json_deserialized_payload["data"]]
    ...
    def convert_dict_to_trade(self, *, input, api_method, symbol):
        return Trade(
            ...
            symbol=symbol,
            exchange_update_time_point=convert_unix_timestamp_milliseconds_to_time_point(unix_timestamp_milliseconds=input["T"]),
            trade_id=input["i"],
            price=input["p"],
            size=input["v"],
            is_buyer_maker=input["S"] == "sell",
        )
    ...
    #     else:
    #         if (
    #             self.instrument_type == BybitInstrumentType.FUTURES
    #             or self.instrument_type == BybitInstrumentType.SWAP
    #             or self.instrument_type == BybitInstrumentType.OPTION
    #         ):
    #             is_long = not pos.startswith("-")
    #         elif self.instrument_type == BybitInstrumentType.MARGIN:
